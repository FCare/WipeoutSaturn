...
ttf_t *track_load_tile_format(char *ttf_name) {
	...
	ttf_t *ttf = malloc(sizeof(ttf_t) + sizeof(ttf_tile_t) * num_tiles);
...
int main(int argc, char *argv[]) {
	...
	texture_t out;
  if (argc != 3) {
		printf("usage: %s library.ttf library.cmp\n", argv[0]);
		return -1;
	}
	ttf_t *ttf = track_load_tile_format(argv[1]);

  cmp_t *cmp = image_load_compressed(argv[2]);
  ...
  uint16_t format = 0x1; //RGB/palette 16 bits
	...
	for (uint32_t i = 0; i < ttf->len; i++) {
		for (int tx = 0; tx < 4; tx++) {
			for (int ty = 0; ty < 4; ty++) {
				...
				for (int j=0; j<images[sub_tile_index]->height; j++) {
					rgba_t* src =  &images[sub_tile_index]->pixels[j*images[sub_tile_index]->width];
					for (int l = 0; l<images[sub_tile_index]->width; l++) {
	...
	offset += ttf->len * ((sizeof(collection_image_t)+0x7)&~0x7);
	collection_image_t *col = (collection_image_t *)malloc(sizeof(collection_image_t) * ttf->len);
  for (int j=0; j<ttf->len; j++) {
		col[j].width = 4 * images[ttf->tiles[0].near[0]]->width;
		col[j].height = 4 * images[ttf->tiles[0].near[0]]->height;
		col[j].offset = offset;
	...
	for (int i=0; i<ttf->len; i++) {
		rgba_t *pixels = malloc(col[i].height*col[i].width *sizeof(rgba_t));
		int idx = 0;
		fseek(f, col[i].offset, SEEK_SET);
		for (int j=0; j<col[i].height; j++) {
			int ty = j/images[ttf->tiles[0].near[0]]->height;
			int py = j%images[ttf->tiles[0].near[0]]->height;
			for (int k=0; k<col[i].width; k++) {
				int tx = k/images[ttf->tiles[0].near[0]]->width;
				int px = k%images[ttf->tiles[0].near[0]]->width;
				uint32_t sub_tile_index = ttf->tiles[i].near[ty * 4 + tx];
				pixels[idx++] = images[sub_tile_index]->pixels[py*images[sub_tile_index]->width + px];
				rgb1555_t pix = SWAP(convert_to_rgb(images[sub_tile_index]->pixels[py*images[sub_tile_index]->width + px]));
		...
		LOGD("extract %s\n", png_name);
		stbi_write_png(png_name, col[i].width, col[i].height, 4, pixels, 0);
		#endif
		free(pixels);
	}
	fclose(f);
