...
void render_push_quads(quads_t *quad, uint16_t texture_index) {
	tris_t t1={.vertices = { quad->vertices[0], quad->vertices[1], quad->vertices[2]} };
	render_push_tris(t1, texture_index);
	tris_t t2={.vertices = { quad->vertices[3], quad->vertices[0], quad->vertices[2]} };
	render_push_tris(t2, texture_index);
}

void render_push_stripe(quads_t *quad, uint16_t texture_index) {
	tris_t t1={.vertices = { quad->vertices[2], quad->vertices[1], quad->vertices[0]} };
	render_push_tris(t1, texture_index);
	tris_t t2={.vertices = { quad->vertices[2], quad->vertices[3], quad->vertices[1]} };
	render_push_tris(t2, texture_index);
}
