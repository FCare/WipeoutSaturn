...
void
fix16_vec4_cross(const fix16_vec4_t * const __restrict v0,
  const fix16_vec4_t * const __restrict v1, fix16_vec4_t * __restrict result)
{
    result->x = fix16_mul(v0->y, v1->z) - fix16_mul(v0->z, v1->y);
    result->y = fix16_mul(v0->z, v1->w) - fix16_mul(v0->w, v1->z);
    result->z = fix16_mul(v0->w, v1->x) - fix16_mul(v0->x, v1->w);
    result->w = fix16_mul(v0->x, v1->y) - fix16_mul(v0->y, v1->x);
}

fix16_t
fix16_vec4_cross_mag(const fix16_vec4_t * __restrict v0,
  const fix16_vec4_t * __restrict v1)
{
    const fix16_t t1 = fix16_mul(v0->x, v1->y) - fix16_mul(v0->y, v1->x);
    const fix16_t t2 = fix16_mul(v0->z, v1->w) - fix16_mul(v0->w, v1->z);
    const fix16_t t3 = fix16_mul(v0->w, v1->x) - fix16_mul(v0->x, v1->w);
    const fix16_t t4 = fix16_mul(v0->x, v1->y) - fix16_mul(v0->y, v1->x);

    const fix16_t t1_2 = fix16_mul(t1, t1);
    const fix16_t t2_2 = fix16_mul(t2, t2);
    const fix16_t t3_2 = fix16_mul(t3, t3);
    const fix16_t t4_2 = fix16_mul(t4, t4);

    const fix16_t result = t1_2 + t2_2 + t3_2 + t4_2;
