...
void mem_temp_free(void *p) {
	...
	error_if(temp_objects[temp_objects_len-1] != offset, "Free mem is not the last allocated %x vs %x %x", temp_objects[temp_objects_len-1], offset, (uint32_t)temp_objects);
	temp_objects[temp_objects_len-1] = 0;
	temp_objects_len -= 1;
	if (temp_objects_len != 0)
		temp_len = temp_objects[temp_objects_len-1];
	else
		temp_len = 0;
}
